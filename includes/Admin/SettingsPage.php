...
final class SettingsPage
{
    ...
    public function register_settings(): void
    {
        ...
        // Only keep these three fields visible
        $this->register_field('api_key', __('Whop API Key', 'whop-woocommerce'), 'render_api_key_field');
        $this->register_field('plan_id', __('Production Plan ID', 'whop-woocommerce'), 'render_plan_id_field');
    ...
    public function sanitize_settings(array $input): array
    {
        ...
        // Ensure $existing is an array to avoid type errors
        if (!is_array($existing)) {
            $existing = $this->config->get_defaults();
        }

        // Preserve hidden settings from existing data
        $sandboxApiKey = $existing['sandbox_api_key'] ?? '';
        $sandboxPlanId = $existing['sandbox_plan_id'] ?? '';
        $checkoutDomain = $existing['checkout_domain'] ?? '';
        $checkoutReturnUrl = $existing['checkout_return_url'] ?? home_url('/checkout/complete');
        $sandboxMode = $existing['sandbox_mode'] ?? 'no';
        $debugMode = $existing['debug_mode'] ?? 'no';

        // Update only the visible settings if provided, otherwise preserve existing
        $apiKey = (isset($input['api_key']) && trim($input['api_key']) !== '') 
            ? sanitize_text_field($input['api_key']) 
            : ($existing['api_key'] ?? '');

        $planId = (isset($input['plan_id'])) 
            ? sanitize_text_field($input['plan_id']) 
            : ($existing['plan_id'] ?? '');

        $webhookSecret = (isset($input['webhook_secret']) && trim($input['webhook_secret']) !== '') 
        ...
        return [
            ...
            'plan_id' => $planId,
            'sandbox_plan_id' => $sandboxPlanId,
            'webhook_secret' => $webhookSecret,
            'checkout_return_url' => $checkoutReturnUrl,
    ...
    {
        echo '<p>' . esc_html__('Configure your Whop production checkout integration settings below.', 'whop-woocommerce') . '</p>';
    }
    ...
    public function render_api_key_field(): void
    {
        ...
        echo '<p class="description">' . esc_html__(
            'Enter your Whop Production API Key.',
            'whop-woocommerce'
        ) . '</p>';
    }
    ...
    public function render_plan_id_field(): void
    {
        ...
        echo '<p class="description">' . esc_html__(
            'Enter the Whop Plan ID customers will purchase.',
            'whop-woocommerce'
        ) . '</p>';
    }
    ...
    public function render_webhook_secret_field(): void
    {
        ...
        echo '<p class="description">' . esc_html__(
            'Enter the Webhook Secret configured in your Whop Dashboard.',
            'whop-woocommerce'
        ) . '</p>';
    }
