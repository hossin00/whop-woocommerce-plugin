...
final class OrderHandler
{
    ...
    public function completeFromWebhookPayload(array $payload, string $webhookId): void
    {
        ...
        // Detailed logging of metadata for debugging
        if (defined('WP_DEBUG') && WP_DEBUG) {
            $this->logger->log('Webhook metadata received.', [
        ...
        $this->validateCartPaymentAmount($order, $payload, $webhookId);
        $this->validateCardDeferredPaymentAmount($order, $payload, $paymentId, $webhookId);
    ...
    {
        $checkoutId =
            $payload['data']['checkout_configuration_id']
            ?? $payload['data']['checkout_configuration']['id']
            ?? $payload['data']['checkout_id']
            ?? $payload['data']['metadata']['checkout_configuration_id']
    ...
    {
        $orderId = $payload['data']['metadata']['wc_order_id'] ?? $payload['data']['metadata']['order_id'] ?? null;
    ...
    private function findOrderByCustomerEmail(string $email): ?WC_Order
    {
        if (! function_exists('wc_get_orders')) {
            return null;
        }
    ...
    /**
     * Cart checkout uses a dynamic inline Whop plan. Complete the WooCommerce
     * order only when Whop reports the exact gross amount and currency stored at
     * checkout creation. Buy Now retains its established reconciliation flow.
     *
     * @param array<string, mixed> $payload
     */
    private function validateCartPaymentAmount(WC_Order $order, array $payload, string $webhookId): void
    {
        if ('yes' !== (string) $order->get_meta('_whop_cart_checkout', true)) {
            return;
        }

        $payment = isset($payload['data']) && is_array($payload['data']) ? $payload['data'] : [];
        $receivedTotal = $payment['total'] ?? null;
        $receivedCurrency = isset($payment['currency']) ? strtolower(sanitize_text_field((string) $payment['currency'])) : '';
        $expectedTotal = (string) get_post_meta($order->get_id(), '_whop_checkout_amount', true);
        $expectedCurrency = strtolower((string) get_post_meta($order->get_id(), '_whop_checkout_currency', true));

        if ($expectedTotal === '') {
            $expectedTotal = wc_format_decimal($order->get_total(), 2);
        }
        ...
        $matches = is_numeric($receivedTotal)
            && wc_format_decimal((string) $receivedTotal, 2) === wc_format_decimal($expectedTotal, 2)
            && $receivedCurrency !== ''
            && hash_equals($expectedCurrency, $receivedCurrency);

        if (! $matches) {
            update_post_meta($order->get_id(), '_whop_payment_validation_status', 'mismatch');
            update_post_meta($order->get_id(), '_whop_payment_validation_expected_total', wc_format_decimal($expectedTotal, 2));
            update_post_meta($order->get_id(), '_whop_payment_validation_expected_currency', $expectedCurrency);

            $this->logger->log('Cart payment amount or currency mismatch; WooCommerce order was not completed.', [
                'order_id' => $order->get_id(),
                'webhook_id' => $webhookId,
                'expected_total' => wc_format_decimal($expectedTotal, 2),
                'expected_currency' => $expectedCurrency,
                'received_total' => is_numeric($receivedTotal) ? wc_format_decimal((string) $receivedTotal, 2) : 'missing',
                'received_currency' => $receivedCurrency !== '' ? $receivedCurrency : 'missing',
            ], 'ERROR');

            throw new InvalidArgumentException(esc_html__('Whop payment amount or currency does not match the WooCommerce order.', 'whop-woocommerce'));
        }

        update_post_meta($order->get_id(), '_whop_payment_validation_status', 'matched');
        update_post_meta($order->get_id(), '_whop_payment_validation_total', wc_format_decimal((string) $receivedTotal, 2));
        update_post_meta($order->get_id(), '_whop_payment_validation_currency', $receivedCurrency);
    }

    /**
     * @param array<string,mixed> $payload
     */
    private function validateCardDeferredPaymentAmount(WC_Order $order, array $payload, string $paymentId, string $webhookId): void
    {
        if ((string) $order->get_meta('_whop_card_payment_state', true) !== 'charge_pending') {
            return;
        }

        $expectedPaymentId = sanitize_text_field((string) $order->get_meta('_whop_card_deferred_payment_id', true));
        $expectedTotal = wc_format_decimal((string) $order->get_meta('_whop_card_expected_total', true), 2);
        $expectedCurrency = strtolower(sanitize_text_field((string) $order->get_meta('_whop_card_expected_currency', true)));
        $payment = isset($payload['data']) && is_array($payload['data']) ? $payload['data'] : [];
        $receivedTotal = $payment['total'] ?? null;
        $receivedCurrency = isset($payment['currency']) ? strtolower(sanitize_text_field((string) $payment['currency'])) : '';

        $matches = $expectedPaymentId !== ''
            && hash_equals($expectedPaymentId, $paymentId)
            && is_numeric($receivedTotal)
            && wc_format_decimal((string) $receivedTotal, 2) === $expectedTotal
            && $expectedCurrency !== ''
            && hash_equals($expectedCurrency, $receivedCurrency);
        ...
        update_post_meta($order->get_id(), '_whop_card_payment_validation_status', 'mismatch');
        $this->logger->log('Deferred Card payment validation failed; WooCommerce order was not completed.', [
            'order_id' => $order->get_id(),
            'webhook_id' => $webhookId,
            'expected_currency' => $expectedCurrency,
            'received_currency' => $receivedCurrency !== '' ? $receivedCurrency : 'missing',
            'payment_id_matches' => $expectedPaymentId !== '' && hash_equals($expectedPaymentId, $paymentId),
        ], 'ERROR');

        throw new InvalidArgumentException(esc_html__('Whop Card payment does not match the WooCommerce order.', 'whop-woocommerce'));
    }
