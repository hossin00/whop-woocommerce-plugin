...
final class HealthCheckService
{
    ...
    public function runChecks(): void
    {
        ...
        $this->results[] = $this->checkCheckoutDomain();
        $this->results[] = $this->checkCheckoutDomainSsl();
    ...
    private function checkPlanId(): array
    {
        ...
        if (trim($planId) === '') {
            return $this->failure('Default plan ID configured', __('Whop default plan ID is not configured.', 'whop-woocommerce'));
        }

        return $this->success('Default plan ID configured', __('Whop default plan ID is configured.', 'whop-woocommerce'));
    }

    /** @return array<string, string> */
    private function checkCheckoutDomain(): array
    {
        $checkoutDomain = $this->config->get('checkout_domain');

        if (trim($checkoutDomain) === '') {
            return $this->failure('Checkout domain configured', __('Checkout domain is not configured.', 'whop-woocommerce'));
        }

        if (filter_var('https://' . $checkoutDomain, FILTER_VALIDATE_URL) === false) {
            return $this->warning('Checkout domain configured', __('Checkout domain does not appear valid.', 'whop-woocommerce'));
        }

        return $this->success('Checkout domain configured', __('Checkout domain is configured.', 'whop-woocommerce'));
    }

    /** @return array<string, string> */
    private function checkCheckoutDomainSsl(): array
    {
        $checkoutDomain = trim((string) $this->config->get('checkout_domain'));

        if ($checkoutDomain === '') {
            return $this->warning('Checkout domain SSL', __('Checkout domain not configured; cannot verify SSL.', 'whop-woocommerce'));
        }

        $url = 'https://' . $checkoutDomain;
        ...
        if (is_wp_error($response)) {
            /* translators: %s is the WordPress error message returned while checking the checkout domain SSL certificate. */
            return $this->failure('Checkout domain SSL', sprintf(__('Unable to verify SSL for checkout domain: %s', 'whop-woocommerce'), $response->get_error_message()));
        }
        ...
        if ($code >= 200 && $code < 400) {
            ...
            return $this->success('Checkout domain SSL', sprintf(__('Checkout domain SSL verified (response %1$d, %2$.2fs).', 'whop-woocommerce'), $code, $duration));
        }

        /* translators: %1$d is the HTTP response code and %2$.2f is the request duration in seconds. */
        return $this->warning('Checkout domain SSL', sprintf(__('Checkout domain responded with HTTP %1$d (%2$.2fs).', 'whop-woocommerce'), $code, $duration));
    }
    ...
    private function checkApiConnection(): array
    {
        ...
        if (! $result['success']) {
            /* translators: %s is the API connection error message returned by the Whop client. */
            return $this->failure('API connection successful', sprintf(__('API connection failed: %s', 'whop-woocommerce'), $result['message']));
        }

        return $this->success('API connection successful', __('API connection to Whop passed.', 'whop-woocommerce'));
    }
